package ru.otus.frontend.hibernate.sessionmanager;

import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.Transaction;
import ru.otus.frontend.core.sessionmanager.SessionManager;
import ru.otus.frontend.core.sessionmanager.SessionManagerException;
...
public class SessionManagerHibernate implements SessionManager {
    ...
    private void checkSessionAndTransaction() {
        if (databaseSession == null) {
            // databaseSession непонятно почему, но после вставки пользователя обнуляется
            databaseSession = new DatabaseSessionHibernate(sessionFactory.openSession());
            return;
